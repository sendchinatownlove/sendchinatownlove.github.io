  ...
  return (
    <div className={classnames(styles.subsection, styles.orderNow)}>
      {showingAltLayout ? (
        <h2
          className={classnames(
            styles.text__toggleMobileDropdown,
            styles.heading__mobileDropdown
          )}
        >
          View Hours and Order
        </h2>
      ) : null}
      <div className={styles.orderNow__hoursSect}>
        ...
        <div className={styles.orderNow__hours}>
          {hours.length > 0 ? (
            ...
            <p className={styles['p--noMargin']}>
              Please call this merchant to confirm open hours.
            </p>
          )}
        </div>
      </div>
      ...
      <p className={styles.orderNow__smaller}>
        ...
        straight from them:
        <span className={styles.orderNow__bold}>
          {phone ? ' ' + phone.phone_number : ' Not Available'}
        </span>
      </p>
      <div className={styles.delivery_container}>
        {phone && (
          <DeliveryButton
            imageUrl={phone.delivery_type.icon_url}
            name={phone.delivery_type.name}
